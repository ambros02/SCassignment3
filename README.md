...
handlers dictionary the handlers for adding and clearing watchpoints were implemented. In the run method, after an operation is executed the progranm checks all watched values, against the actual values and tells the user which one and lets the user interact in the same position. 
.</p>
